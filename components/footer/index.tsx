  ...
  return (
    <footer className="bg-primary-40">
      <div className="w-[88%] md:w-[93%] mx-auto pt-10 pb-4 md:pb-6">
        ...
        {/* footer link */}
        <div className="flex flex-col md:flex-row items-center md:items-stretch mb-9 md:mb-0">
          <div className="mt-[3.44rem] w-full md:w-1/3 flex flex-col md:flex-row gap-6 md:gap-[3.44rem] order-2 md:order-first">
          ...
          <Link href="/" className="md:mt-[2.81rem] mx-auto hover-opacity order-first md:order-2 mb-6 md:mb-0">
            <Image src={logo} alt="" className="w-[14.12rem] h-[4.14rem]" />
          </Link>
          ...
          <div className="w-full md:w-[26.125rem] mt-11 ml-auto order-last">
            <strong className="text-1 font-semibold flex flex-row items-center leading-1.2 -tracking-0.0025 text-white mb-[0.44rem] gap-[0.56rem]">
              <Image src={building} alt="" className="block md:hidden" />
              Chi nhánh văn phòng
            </strong>
            <div className="w-full h-[0.0625rem] bg-primary-30" />
            <div className="mt-6 md:mt-4 flex flex-row md:flex-col gap-y-6 md:gap-4 flex-wrap">
              {footerOfficeLocations.map(item => {
                return (
                  <div key={item.name} className="basis-1/2 md:basis-auto flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-4">
                    <p className="md:basis-[25%] text-0.875 leading-1.5 md:leading-1 text-white font-bold md:font-normal">{item.name}</p>
                    <p className="md:basis-[40%] text-0.75 md:text-0.875 leading-1.2 md:leading-1 text-primary-10 md:text-white">{item.detail}</p>
        ...
        {/* footer information */}
        <div className="flex flex-col md:flex-row md:items-end gap-9 md:gap-[12.31rem] mb-9 md:mb-[1.88rem]">
          <FooterSocialLinkGroup className="hidden md:flex order-first" />
          <div className="flex flex-col md:gap-1 order-2 md:order-1">
            <strong className="text-1 font-semibold flex md:hidden flex-row items-center leading-1.2 -tracking-0.0025 text-white mb-[0.44rem] gap-[0.56rem]">
              <Image src={tel} alt="" className="block md:hidden" />
              Hotline
            </strong>
            <div className="block md:hidden w-full h-[0.0625rem] bg-primary-30 mb-4" />
            <p className="text-1 text-white leading-1.5">Tel: 024 3971 6229</p>
            <p className="text-1 text-white leading-1.5">Tel: 028 3929 3995</p>
          </div>
          <div className="flex flex-col md:gap-1 order-1 md:order-2">
            <strong className="text-1 font-semibold flex md:hidden flex-row items-center leading-1.2 -tracking-0.0025 text-white mb-[0.44rem] gap-[0.56rem]">
              <Image src={card} alt="" className="block md:hidden" />
              Liên hệ
            </strong>
            <div className="block md:hidden w-full h-[0.0625rem] bg-primary-30 mb-4" />
            <p className="text-1 text-white leading-1.5">Hotline chung: 098 870 9698</p>
        ...
        <div className="flex md:mt-[1.6rem]  flex-row items-center">
          <div className="flex flex-row gap-6 items-center">
            <p className="text-0.75 leading-1.5 md:leading-1 text-white">@2023 DucAnh Educonnect</p>
          ...
          <div className="hidden md:flex flex-row items-center gap-8 ml-auto">
            <Link href="/" className="text-white text-0.75 font-bold leading-1 tracking-0.0075 capitalize">Policy</Link>
            <ScrollToTopButton />
          </div>
        </div>
      </div>
