...
export default function GsapDesktop() {
  ...
  return (
    <div ref={divRef} className="hidden md:block w-[76.4rem] h-[44.4rem] bg-transparent absolute top-[44%] left-1/2 -translate-x-1/2 z-10">
      <Image ref={bookRef} src={section3BookImage} alt="tôn chỉ làm việc" className="absolute left-6 top-0 w-[38rem] h-[44rem] object-cover" />
      <div ref={textRef} className="absolute top-[13.12rem] left-[2rem] w-[17.8125rem] flex flex-col">
        <strong className="text-right text-1 font-bold uppercase text-white mb-4">ĐỨc anh educonnect</strong>
        <h1 className="text-right text-[4.5rem] font-extrabold leading-1 tracking-[-0.045rem] text-white">Tôn chỉ làm việc</h1>
      </div>
      <ul ref={menuRef} className="absolute top-[8rem] right-0 flex flex-col [&_li]:mb-[1.88rem]">
        {menuItems.map(item => {
          return <MenuItem key={item.text}>{item.text}</MenuItem>
        })}
      </ul>
      <Image ref={decoRef} src={section3Deco} alt="tôn chỉ làm việc" className="absolute bottom-0 left-[6.75rem] -z-10 w-[78.875rem] h-[9.1875rem]" />
      <div ref={buttonContainerRef} className="absolute left-[14rem] bottom-[9rem] size-[5.6875rem]">
        <Image data-role="button-click-deco" src={buttonClick} alt="tôn chỉ làm việc" className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-[5.6875rem]" />
        <Image src={mouse} alt="tôn chỉ làm việc" className="absolute top-1/2 left-1/2" />
      </div>
    </div>
  )
}